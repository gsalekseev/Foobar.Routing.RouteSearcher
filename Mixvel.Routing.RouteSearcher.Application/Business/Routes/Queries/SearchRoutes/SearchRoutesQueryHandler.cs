...
namespace Mixvel.Routing.RouteSearcher.Application.Business.Routes.Queries.SearchRoutes
{
    public class SearchRoutesQueryHandler : IRequestHandler<SearchRoutesQuery, SearchRoutesResultDto>
    {
        ...
        {
            var providersRequest = _mapper.Map<RouteProviderRequest>(request);
            ...
            if (cacheRequired)
            {
                ...
                routes = FilterCachedRoutes(providersRequest, cachedRoutes);
            }
            else
            {
                var providersRoutes = await _aggregator.AggregateFromProviders(providersRequest, cancellationToken);
